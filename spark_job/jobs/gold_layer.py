...
from pyspark.sql.functions import from_json, col, current_timestamp, broadcast
import logging
import os
...
def gold_dim_time(silver_timestamp):
    df = (
        ...
        silver_timestamp
        .select("order_time")
        .withColumn("order_time", F.to_timestamp("order_time"))      # ép kiểu rõ ràng
        .dropDuplicates()                                           # hoặc .withWatermark...
        .withColumn("time_id",  F.date_format("order_time", "HHmm"))# để dạng string
...
def gold_dim_pizza(silver_pizza_catalog: DataFrame) -> DataFrame:
    return (
        ...
        .withColumn("pizza_sk", F.crc32(F.col("pizza_id")))  # safe for streaming
    )
# def gold_dim_pizza(silver_pizza_catalog: DataFrame) -> DataFrame:
...
#         silver_pizza_catalog
#         .withColumnRenamed("pizza_name_id", "pizza_id")
#         .withColumn("pizza_sk", F.monotonically_increasing_id())  # surrogate key
#     )
# def gold_dim_pizza(silver_pizza_catalog: DataFrame) -> DataFrame:
#     return (
#         silver_pizza_catalog
#         .withColumnRenamed("pizza_name_id", "pizza_id")
#         .withColumn("pizza_sk", F.crc32(F.col("pizza_name")))  # surrogate key dạng chuỗi UUID
#     )
# Mỗi hàng đại diện cho 1 pizza trong một đơn hàng
...
#     fact = (
#         order_items.alias("f")
#         # join dim tables để lấy surrogate key, chuẩn hóa thành INT
#         .join(dim_date.select("order_date", "date_id"),  "order_date")
#         .join(dim_time.select("order_time", "time_id"),  "order_time")
#         .join(dim_pizza.select("pizza_id",  "pizza_sk"), "pizza_id")
#         .select(
#             F.to_timestamp("order_date").alias("order_date"),
#             F.col("date_id").cast("int"),
#             F.col("time_id").cast("int"),
#             F.col("pizza_sk").cast("int"),
...
def gold_fact_order_item(silver_order_items: DataFrame, dim_date: DataFrame,
    ...
    fact = (
        ...
        .join(broadcast(dim_pizza.select("pizza_id", "pizza_sk")), "pizza_id")
        .select(
            F.to_timestamp("order_date").alias("order_date"),
            F.col("date_id").cast("int"),
            F.col("time_id").cast("int"),
...
# def gold_fact_daily_sales(fact_order_item: DataFrame) -> DataFrame:
#     return (
#         fact_order_item
#         .withWatermark("order_date", "1 day")
#         .groupBy("date_id")
#         .agg(
#             F.sum("quantity").alias("qty_sold"),
#             F.sum("total_price").alias("sales_amount"),
#             F.approx_count_distinct("pizza_sk").alias("distinct_pizzas") # countDistinct
#         )
#     )
...
logging.info("---------------read done for dim_date -----------")
dim_time = gold_dim_time(silver_ts)
logging.info("---------------read done for dim_date -----------")
dim_pizza = gold_dim_pizza(silver_catalog)
logging.info("---------------read done for dim_date -----------")
fact_order_item = gold_fact_order_item(silver_order,dim_date = dim_date, dim_time=  dim_time, dim_pizza = dim_pizza)
logging.info("---------------read done for gold_fact_order_item -----------")


silver_catalog.writeStream \
    .format("console") \
    .outputMode("append") \
    .trigger(once=True) \
    .start() \
...
# đã test và pizza_sk không chứa duplictate
...
#     silver_ts = read_delta_stream(spark, "lakehouse", "silver", "silver_timestamp")
#     write_stream_to_lake(gold_dim_date(silver_ts), bucket = BUCKET, layer = LAYER, table_name ="gold_dim_date")

# elif target == "gold_dim_time":
#     silver_ts = read_delta_stream(spark, "lakehouse", "silver", "silver_timestamp")
#     df = gold_dim_time(silver_ts)
#     write_stream_to_lake(df, bucket = BUCKET, layer = LAYER, table_name ="gold_dim_time")

# elif target == "gold_dim_pizza":
#     silver_catalog = read_delta_stream(spark, "lakehouse", "silver", "silver_pizza_catalog")
#     write_stream_to_lake(gold_dim_pizza(silver_catalog), bucket = BUCKET, layer = LAYER, table_name ="gold_dim_pizza")

# elif target == "gold_fact_order_item":
#     silver_order = read_delta_stream(spark, "lakehouse", "silver", "silver_order_items")
#     write_stream_to_lake(gold_fact_order_item(silver_order), bucket = BUCKET, layer = LAYER, table_name ="gold_fact_order_item")

# elif target == "gold_fact_daily_sales":
#     silver_order = read_delta_stream(spark, "lakehouse", "silver", "silver_order_items")
#     fact_order_item = gold_dim_date(silver_order)
#     write_stream_to_lake(gold_fact_daily_sales(fact_order_item), bucket = BUCKET, layer = LAYER, table_name ="gold_fact_daily_sales")
